...
def normalize(data, exclude=None):
    norm_cols = [n for n, c in data.items() if len(np.unique(c)) > 2]
    n = data.shape[0]
...
#                 print(self.df.isnull().values.any())
                ...
                index_samples_inplace=random.sample(range(0, np.shape(self.df)[0]), swap_amt) # rows to be put in place of the 
...
#                     b.append(binary.item())
                    ...
                    if idx%10==0:print('Train: ',epoch,idx,r(sum(L)/len(L)))
...
torch.save(checkpoint, '/home/Drive/rahul/PortoSeguroSafeDriverPrediction/'+save_initial)
